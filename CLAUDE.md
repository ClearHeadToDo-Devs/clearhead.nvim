...
Done right, we can make functionality that is completely unmatched by other editors. Done wrong, and it will feel like you are fighting the editor to do even the smallest things.
...
The suite uses upstream Busted with `nlua`, so every test runs inside a real Neovim process without depending on Plenary's retired test adapter.
